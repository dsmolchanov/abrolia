...
4. **Исходящие** — семейным адресатам tracked-send; всем прочим только через
   staged approval. Транспортная таксономия донора: redirect → `failed`,
   ReadTimeout → `SendOutcomeUnknown` (никогда слепого повтора), ConnectError →
   `failed`.
