...
| T27 | Раскрытие relay-адреса пересылки (Gmail-опция b) | посторонний узнаёт `fwd-…@abrolia.com` и шлёт письма прямо в relay, минуя Gmail | адрес — HMAC под lookup-ключом, 26 base32-символов, показывается только владельцу; всё входящее в relay — untrusted-контент, который даёт лишь предложения: та же модель, что и публичный `@abrolia.com` inbox, адрес — гигиена, а не граница; семейный managed local part `fwd-…` запрещён | `tests/test_gmail_forwarding_runtime.py`, `test_assistant_address_per_household.py` |
| T28 | Подделанное «подтверждение пересылки» | письмо в relay с чужой ссылкой, чтобы владелец открыл её | confirmation распознаётся только от отправителя Google с ровно одной https-ссылкой `mail-settings.google.com/mail/vf-…`, называющей адрес агента; всё остальное — обычное письмо; ссылка показывается владельцу в чате и никогда не запрашивается runtime | classifier tests (`forwarding.py`) |
| T29 | Подделанное контрольное письмо | письмо «Abrolia forwarding check …» в relay, чтобы скрыть сломанную пересылку | засчитывается только токен `HMAC(webhook key, дата)`, который runtime сам отправил последним; чужой токен divert'ится без события и ничего не доказывает; влияет разве что на статус здоровья, не на данные | `tests/test_gmail_forwarding_health.py` |
