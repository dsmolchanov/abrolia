...
1. **Выполнено 2026-08-12** ([`lawful-bases.md`](lawful-bases.md), р. 3):
   ...
   услуги. Остаются два реализуемых шага до реальных данных: отдельная
   consent-цель для содержимого household'а (существующий
   `special_category_content_restriction` её не заменяет) и запись проверки
   ст. 9(4) по стране пилота.
