...
Ст. 35(4) и необходимость консультации по ст. 36 рассмотрены 2026-08-12 —
см. р. 5, п. 7. Национальный список надзорного органа сверяется по стране
пилота вместе с проверкой ст. 9(4) ([`lawful-bases.md`](lawful-bases.md), р. 3).
...
| R7 | Особые категории (здоровье, религия) в письмах | третьи лица, дети | высокий | условие ст. 9(2)(a) выбрано 2026-08-12 для взрослых и собственных детей household'а (согласие носителя родительской ответственности); особые категории о третьих лицах — вне scope услуги; versioned запрет на отправку таких материалов + удаление по обращению; не извлекаем и не индексируем такие атрибуты; короткий TTL ([`lawful-bases.md`](lawful-bases.md), р. 3) | **средний, принят**: для in-scope субъектов условие есть, consent-цель `special_category_household_content` требуется fail-closed в real-email rollout, проверка ст. 9(4) по DE/IT/NL/ES проведена. Остаётся риск случайно доставленного материала о третьих лицах, для которого условия нет и не будет — он удаляется по обращению |
...
1. **Выполнено 2026-08-12** ([`lawful-bases.md`](lawful-bases.md), р. 3):
   условие ст. 9(2)(a) выбрано и задокументировано для взрослых и собственных
   детей household'а; особые категории о третьих лицах вынесены за scope
   услуги; проверка ст. 9(4) проведена по DE/IT/NL/ES; отдельная consent-цель
   `special_category_household_content` реализована и требуется fail-closed в
   real-email rollout. Для итальянского household дополнительно сверяются
   действующие misure di garanzia Garante (ст. 2-septies).
...
6. Заполнены реквизиты контролёра (Axiom Atlas, LLC, Делавэр, США) и адрес для
   запросов субъектов (`help@abrolia.com`); notice содержит полный набор
   ст. 13/14, включая право на жалобу по ст. 77. Наименование конкретного
   надзорного органа предпосылкой не является: ст. 13(2)(d) требует сообщить о
   праве подать жалобу, а не назвать орган, и субъект в любом случае вправе
   обратиться по месту жительства, работы или предполагаемого нарушения.
   Остаётся открытым назначение представителя в Союзе по ст. 27 — это
   самостоятельная обязанность ([`processors.md`](processors.md), р. 2 п. 4a).
7. DPIA проверена на предмет ст. 35(4) и необходимости консультации по ст. 36.
   **Рассмотрено 2026-08-12** (`/s/ Product owner (CEO), выступая как counsel
   контролёра, Axiom Atlas, LLC`). Роли названы обе: план закрепляет ревью за
   counsel, отдельного внешнего юриста у контролёра нет, и эту роль исполняет
   владелец продукта. Ревью и решение исходят от одного лица — независимой
   проверки нет, и это ограничение записи, а не её обход.
   Ст. 36 не применяется: остаточные риски после мер не остаются высокими —
   единственный ранее блокирующий R7 снят выбором условия ст. 9(2)(a) и
   выведением данных третьих лиц за scope. Ст. 35(4): национальный список
   надзорного органа сверяется по стране пилота вместе с проверкой ст. 9(4).
   **DPO не требуется** (ст. 37(1)): Abrolia не публичный орган; ни регулярный
   систематический мониторинг субъектов в крупном масштабе, ни обработка особых
   категорий в крупном масштабе не являются основной деятельностью — пилот
   ограничен 5–20 домохозяйствами, особые категории выведены за scope, а
   профилирование субъектов отсутствует. Позиция пересматривается при выходе за
   пределы пилота.
