...
| S4 | Бэкапы SQLite (снапшоты на Fly volume; отдельный object-storage provider не используется в пилоте) | копии S1 и отдельная копия S14; runtime и control plane имеют разные ключи и restore-процедуры | Fly.io `ams` (EU, NL), контур P2 | AES-256, отдельный ключ per-household/control-plane | оператор |
| S5 | Nerve inbox (email-опции a/c; relay для опции b) | `@abrolia.com` или домен семьи: входящие письма, треды, метаданные вложений. Для Gmail-опции b — скрытый relay-ящик `fwd-…@abrolia.com`, куда Gmail пересылает входящие семьи, плюс ежедневное контрольное письмо relay→агент (без семейных данных) | Fly.io `iad` (US) | провайдер | оператор (тот же владелец) |
...
| S11 | Отдельный Gmail агента (email-опция b) | отдельный Google-аккаунт, который семья создаёт для ассистента; Gmail API только `gmail.send` (без чтения ящика), не личный ящик владельца; входящие попадают к нам через пересылку Gmail в relay-ящик S5. Состояние пересылки (`pending`/`active`/`stale`, метки времени контрольных писем, ссылка подтверждения Gmail до показа владельцу) — таблица `gmail_forwarding_state` в S1 | Google | провайдер | семья |
| S12 | Логи приложения и метрики | ID, статусы, тайминги, размеры — **без содержимого**; отдельный logs/alerts provider не используется в пилоте | Fly.io `ams`, platform logs в контуре P2 | провайдер | оператор |
...
| Email identity/reservation/activation (`email_identities`, `email_address_reservations`, `email_activation_receipts`) | encrypted address/provider subject, masked address, provider/secret refs and digests, scopes, state/version, local-part reservation, email-check timestamp, separate runtime-health timestamp/status and version-bound attention ownership; без provider credential | owner, email provider projector, runtime activation | S14 | identity/receipt: жизнь account + **30 дн.**; released/expired reservation: **24 ч.** | ✔ | ✔ |
| Channel preferences (`channel_preferences`) | household/actor primary/fallback verified bindings, household-level MVP with per-actor schema ready; fallback is verified owner contact email never equal to agent inbox | owner channel selection + test receipt | S14 | жизнь household/account + **30 дн.** | ✔ | ✔ |
| Token usage (`usage_daily`) | per-household/day prompt/completion tokens + usd_estimate, без содержимого писем; operational metrics | система (model usage reply) | S1 | **90 дн.** | ✖ | ✔ |
|| Email secret install receipt (`email_secret_installs`) | job id, household id, secret name, namespace ref, installed_at — **без secret value** | control-plane worker after SecretSink install | S14 | жизнь household/account + **30 дн.** | ✖ | ✔ |
...
| Onboarding/channel consent receipts (`consent_receipts`) | special-category content restriction acknowledgement; shared-WhatsApp privacy notice; отдельный dedicated-QR risk consent; purpose, account, locale, text version/hash, accepted/revoked times | account owner onboarding | **S14 — авторитетный append-only источник**; S1 получает только `receipt_id`, digest и enforcement mirror | срок действия + **3 года после отзыва** | ✔ | частично: доказательный факт/версия/hash сохраняются, лишнее содержание стирается |
...
| Dedicated Gmail агента (опция b) | Google; пересланные входящие — Nerve `iad` (US), Resend (inbound, US) | грант выдаёт семья через OAuth; пересылку включает семья в Gmail | ⏳ sensitive-scope verification gate (CASA не требуется: read-скоупа нет) | отдельный аккаунт агента; только `gmail.send`; refresh token encrypted, disconnect отзывает grant; relay-адрес неугадываемый (тот же threat model, что и публичный inbox) |
