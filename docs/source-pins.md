...
Порядок: **push ветки `agent/foundation-activation` → перепин на `a02ab33`
отдельным коммитом** (с перечнем того, что меняется в портируемом слое).
Промежуточный `1633ffe` не выбирается: это лишь первый коммит серии
исправлений. До push порт кода из непубличных коммитов не начинается.
