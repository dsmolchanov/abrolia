...
CI + the `codex-review-window` check are the only hard merge gates. The Codex
gate is fail-closed: every non-draft PR head must receive a Codex review, and
P1 `[BLOCKER]` findings keep auto-merge blocked until Auto-fix pushes a fix and
Codex reviews the new head as green.
