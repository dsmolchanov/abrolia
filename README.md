...
- **Controller**: Axiom Atlas, LLC (Delaware, USA), registered office 131 Continental Dr, Suite 305, Newark, DE 19713. Data-subject requests: `help@abrolia.com`. No Art. 27 GDPR Union representative has been designated yet, which is one of the reasons real family data is not processed — see [`docs/privacy/privacy-notice-en.md`](docs/privacy/privacy-notice-en.md).

Status: pilot MVP in production since 2026-09-08 for the managed `@abrolia.com` and family-domain email options, with real family data; the open compliance items (Fly.io DPA, transfer assessments, the Art. 27 representative) are stated in the privacy notices where they apply. The separate-agent Gmail option is offered to Google test users only: it requests `gmail.send` alone — incoming mail reaches the assistant through Gmail forwarding the family turns on — and opens to everyone after Google's sensitive-scope verification (`docs/google-verification.md`).
