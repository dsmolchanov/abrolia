...
**Статус: Gate −1 закрыт 2026-08-03 для работы на синтетических данных.** Оба ручных пункта приняты владельцем (детали — в самих пунктах выше); ревизии гейта — `thoughts/shared/implementations/2026-08-02-family-ops-assistant-mvp-validation.md`. Зелёные автопроверки означают лишь, что текущие образцы удовлетворяют текущим правилам, поэтому санитайзер и его тесты остаются живым артефактом. **Реальные данные, включая ящик владельца, заблокированы** до закрытия всех предпосылок `docs/privacy/dpia.md`, р. 5 — в первую очередь условия ст. 9(2) и ревью юриста.
...
#### Manual Verification:
- [ ] Синтетическое немецкое школьное письмо через inject-eml → корректная карточка на русском → ✅ → напоминание сработало (сквозной путь покрыт `tests/test_pipeline.py` на фейковом транспорте; ручная проверка — с живым ботом, runbook в README)
